...
function buildSet(input) {
  ...
  input.split(',').forEach(i => {
    let direction = i.substring(0,1);
    let times = Number(i.substring(1));
...
export function findDistance(inOne, inTwo) {
  const set1 = buildSet(inOne);
  const set2 = buildSet(inTwo);

  let minDistance = Number.MAX_VALUE;
  let intersection = new Set([...set1].filter(x => set2.has(x)))

  intersection.forEach((value: string) => {
    let [x, y] = value.split(',');
    let distance = Math.abs(Number(x)) + Math.abs(Number(y));
...
const result = findDistance(wireOneInput, wireTwoInput);
console.log(result);
